...
from utils_sort import sort_by_fire_type_year
...
def er_adj(df,efcoldf):
   ...
   Emission Ratio adjustment calculation.
    ...
    for fire_type in set(efcoldf['fire_type'].unique())-{'peat'}: 
        ...
        fl=pd.read_sql('select * from bkdb_compound_flaming_combustion_type', con=bk_db)
        ...
        for l in lab:
            ...
            co2_cor[l+'_ER_ADJ']=round(co2_cor * fieldavg.iloc[co2_ind] ,4)
            
            co_cor=df[l][df.index.isin(sml_ind)]/df[l].iloc[co_ind]
            co_cor[l+'_ER_ADJ']=round(co_cor * fieldavg.iloc[co_ind],4)
            
            cor=pd.DataFrame()
            cor[new_col]=co2_cor[new_col].append(co_cor[new_col])
